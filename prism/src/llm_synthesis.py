...
def call_free_fallback_model(prompt, failed_model_label):
        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "JSON object matching the requested schema keys."
                ),
            },
    ...
    try:
        response = requests.post(url, headers=headers, json=payload, timeout=45)
        if response.status_code != 200:
            note = f"{failed_model_label} failed, and the free fallback model ({FREE_FALLBACK_MODEL}) also returned HTTP {response.status_code}."
            print(f"[PRISM] {note}")
            return _placeholder_with_warning(note)
        ...
        if start_idx == -1 or end_idx == -1:
            raise ValueError("No parsable JSON in free fallback model output.")
        ...
        note = f"{failed_model_label} failed, and the free fallback model ({FREE_FALLBACK_MODEL}) also failed ({e})."
        print(f"[PRISM] {note}")
        return _placeholder_with_warning(note)
